...
//validamos que el formulario contenga almenos un dato

if( !empty($_POST)){
    $name = $_POST['name'];
    $image = $_POST['image'];
    $osType = $_POST['osType'];
    $basedOn = implode(",",array($_POST['basedOn']));
    $origin = $_POST['origin'];
    $arch = implode(",",array($_POST['arch']));
    $desktop = implode(",",array($_POST['desktop']));
    ...
    if( empty($name) ){
        $errors['name']['empty_name'] = 'El campo nombre no puede ir vacio';
    }


    if( empty($web) ){
        $errors['web']['empty_web'] = 'El campo web no puede ir vacio';
    }


    if( empty($description) ){
        $errors['description']['empty_description'] = 'El campo descripcion no puede ir vacio';
    }


    if ( empty($errors) ) {

        $sql = "INSERT INTO distro (arch, based, category, description, desktop, doc, error_tracker, forums, image, main_page, name, origin, ostype, status, version) 
              VALUES (:arch, :based, :category, :description, :desktop, :doc, :error_tracker, :forums, :image, :main_page, :name, :origin, :ostype, :status, :version)";
        ...
        //Devolvemos la aplicacion a la pagina de inicio
...
<div class="container">
    ...
    <form action="" method="post">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="inputForum" name="forum"
                   placeholder="Distro Official Forum Website">
        </div>
        <div class="form-group">
            <label for="inputError">Error Tracker</label>
            <input type="text" class="form-control" id="inputError" name="error"
